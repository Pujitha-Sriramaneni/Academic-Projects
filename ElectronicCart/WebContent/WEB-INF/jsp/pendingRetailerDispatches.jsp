...
<body>
	<form:form name="initmateRetailerForm"
		action="retailerMessageDispatchIntimation.html" method="post">
				...
				<tbody>
					<c:forEach var="custCart" items="${customerCarts}">
						<tr>
							...
							<th><c:out value="${custCart.customerId}"></c:out></th>
							<th><c:out value="${custCart.quantity}"></c:out></th>
							<th><c:out value="${custCart.totalValue}"></c:out></th>
							<th>
								<button id="${custCart.customerCartId}"
									value="${custCart.customerCartId}~${custCart.retailerProduct.retailerId}"
									onclick='messageFuncRetailer(this)' style="color: purple;">Message</button>
							</th>
						</tr>
					</c:forEach>
				</tbody>
	...
	<script type="text/javascript">
		function messageFuncRetailer(userValue) {
